	...
	*/
	...
	function validate_amazon_keys() {
		...
		if ( '' == $('input[name*="AccessKeyID"]').val() ) {
			$('input[name*="AccessKeyID"]').val( 'aateam demo access key' );
		}
		if ( '' == $('input[name*="SecretAccessKey"]').val() ) {
			$('input[name*="SecretAccessKey"]').val( 'aateam demo secret access key' );
		}
		//$('input[name*="AffiliateID"]').each(function() {
		//	if ( '' == $(this).val() ) {
		//		$(this).val( 'aateam' );
	...
	function amzCheckAWS( that )
	{
		...
		var associateTag = getAssociateTagByCountry( $('select[name*="country"]').val() );
	...
	function getAssociateTagByCountry( country ) {
		var asoctags = [];
		var countryConvert = {
			'uk': 'co.uk',
			'jp': 'co.jp',
		...
		//WooZoneLite-wizard-AffiliateID-com
		$('input[name*="AffiliateID"]').map(function(idx, elem) {
			//return $.trim( $(this).val() ) != '';
			var aff_val = $(elem).val(),
					aff_country = $(elem).attr('id').replace('WooZoneLite-wizard-AffiliateID-', '');
			aff_country = misc.hasOwnProperty( countryConvert, aff_country ) ? countryConvert[ aff_country ] : aff_country;
			//console.log( aff_country, aff_val );
			asoctags[ aff_country ] = aff_val;
		});

		//console.log( asoctags[ country ], asoctags );
		return asoctags[ country ];
	}
