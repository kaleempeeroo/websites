	...
	class WooZoneLiteWizard
	{
		...
		*/
		...
		public function setupAmazonHelper( $params=array(), $pms=array() ) {
			...
			foreach ( $params as $key => $val ) {
				if ( in_array($key, array(
					'AccessKeyID', 'SecretAccessKey', 'country', 'main_aff_id', 'associateTag', //amazon
		...
		public function wizard_enqueue_scripts() {
			...
			//:: styles
			wp_enqueue_style( $this->alias_wz . '-google-lora',  WooZoneLite_asset_path( 'css', $protocol . '://fonts.googleapis.com/css?family=Lora:400,700', true ), array(), WooZoneLite_asset_version( 'css' ) );
			wp_enqueue_style( $this->alias_wz . '-google-lato',  WooZoneLite_asset_path( 'css', $protocol . '://fonts.googleapis.com/css?family=Lato:400,700', true ), array(), WooZoneLite_asset_version( 'css' ) );

			wp_enqueue_style( $this->alias_wz . '-bootstrap', WooZoneLite_asset_path( 'css', $this->module_folder . 'css/bootstrap.min.css', true ), array(), WooZoneLite_asset_version( 'css' ) );
			wp_enqueue_style( $this->alias_wz . '-demo', WooZoneLite_asset_path( 'css', $this->module_folder . 'css/demo.css', true ), array(), WooZoneLite_asset_version( 'css' ) );
			wp_enqueue_style( $this->alias_wz . '-wizard', WooZoneLite_asset_path( 'css', $this->module_folder . 'css/wizard.css', true ), array(), WooZoneLite_asset_version( 'css' ) );
			wp_enqueue_style( $this->alias_wz . '-responsive', WooZoneLite_asset_path( 'css', $this->module_folder . 'css/responsive.css', true ), array(), WooZoneLite_asset_version( 'css' ) );

			//:: scripts
			wp_enqueue_script( $this->alias_wz . '-use-fontawesome', WooZoneLite_asset_path( 'js', $protocol . '://use.fontawesome.com/5a26c12a15.js', true ), array(), WooZoneLite_asset_version( 'js' ) );

			wp_enqueue_script( 'jquery' );
			wp_enqueue_script( $this->alias_wz . '-hashchange', WooZoneLite_asset_path( 'js', $this->the_plugin->cfg['paths']['freamwork_dir_url'] . 'js/hashchange.min.js', true ), array( 'jquery' ), WooZoneLite_asset_version( 'js' ) );
			wp_enqueue_script( $this->alias_wz . '-main',  WooZoneLite_asset_path( 'js', $this->module_folder . 'js/main.js', true ), array( 'jquery', $this->alias_wz . '-hashchange' ), WooZoneLite_asset_version( 'js' ) );
			...
			if ( 'finished' == $current_step ) {
				...
				if ( isset($save_opt['AccessKeyID']) && isset($save_opt['SecretAccessKey'])
					...
					foreach ( $demo_keys as $demokey ) {
						$_demokey = "{$demokey[0]}|||{$demokey[1]}";
						if ( ! in_array( strtoupper(md5(base64_encode($_demokey))), array('1D8CCE0EA0831C8373F042FCC625032B') ) ) {
							$AccessKeyID = 'aateam demo access key';
							$SecretAccessKey = 'aateam demo secret access key';
						}
		...
		public function WooZoneLiteAffIDsHTML() {
			//global $WooZoneLite;
		...
		public function WooZoneLite_amazon_countries( $what='array' ) {
			//global $WooZoneLite;
			$WooZoneLite = $this->the_plugin;
		...
		public function WooZoneLite_asof_font_size($min=0.1, $max=2.0, $step=0.1) {
			$newarr = array();
