	...
	class WooZoneLiteAssetDownload
	{
		...
		*/
		public function __construct( $is_cron=false )
		{
			...
			$this->settings = $WooZoneLite->settings();
		...
		*/
		private function printBaseInterface()
		{
			global $wpdb;
?>
		<?php echo WooZoneLite_asset_path( 'css', $this->module_folder . 'app.assets_download.css', false ); ?>
				...
				<section class="WooZoneLite-main">
					...
					<div class="panel panel-default WooZoneLite-panel">
						...
						<div class="panel-body WooZoneLite-panel-body">
							...
							<!-- Content Area -->
							<div class="WooZoneLiteAssetDownload" id="WooZoneLite-content-area">
								<div class="WooZoneLite-grid_4">
									<div class="WooZoneLite-panel">
										<div class="WooZoneLite-panel-content">
											<form class="WooZoneLite-form" action="#save_with_ajax">
												<div class="WooZoneLite-form-row WooZoneLite-table-ajax-list" id="WooZoneLite-table-ajax-response">
												<?php
												WooZoneLiteAjaxListTable::getInstance( $this->the_plugin )
													->setup(array(
														'id' 				=> 'WooZoneLiteAssetDownload',
														'show_header' 		=> true,
														'search_box' 		=> false,
														'items_per_page' 	=> 10,
														'post_statuses' 	=> array(
								...
								<div class="WooZoneLite-grid_4">
									<div class="WooZoneLite-panel">
										<div class="WooZoneLite-panel-header">
											<span class="WooZoneLite-panel-title">
												' . __('OR create cron job for doing the assets download jobs', $this->the_plugin->localizationName) . '
											</span>
										</div>
										<div class="WooZoneLite-panel-content">
											<div class="WooZoneLite-sync-details">
		...
		<?php echo WooZoneLite_asset_path( 'js', $this->module_folder . 'app.assets_download.js', false ); ?>
...
<?php
		...
		private function get_asset_generic( $asset_id=0, $nb_dw='all', $post_id=0, $include_variations=true, $inprogress=false, $include_err=false, $include_invalid_post=false ) {
			...
			$q .= ";";
			//var_dump('<pre>', $q , '</pre>'); echo __FILE__ . ":" . __LINE__;die . PHP_EOL;
		...
		private function upd_asset_db( $asset, $first_item=false ) {
			...
			if ( !is_object($asset) && is_int($asset) && ($asset > 0) ) {
				...
				$q = "select a.id, a.post_id, a.asset, a.download_status, a.hash, a.media_id, b.title, b.type, b.post_parent from " . $tables['assets'] . " as a left join " . $tables['products'] . " as b on a.post_id = b.post_id where 1=1 and a.id = $asset;";
			...
			$this->the_plugin->import_stats_update_imgdw( array(
				'post_id' 				=> $post_parent ? $post_parent : $post_id,
				'duration_img_dw'		=> $duration,
				'duration_img_nb_dw'	=> 1,
			));
		...
		public function cronjob( $pms, $return='die' ) {
			...
			if ( $this->the_plugin->is_remote_images ) {
				$ret = array_merge($ret, array(
					'status'            => 'done',
					'msg' 				=> 'you are using remote images option, so there is no need to download assets.',
				));
				return $ret;
			}
			...
			$cronNbImages = 2; //(int) $this->settings['cron_number_of_images'];
			$cronNbImages = $cronNbImages <= 0 || $cronNbImages > $max_images ? $max_images : $cronNbImages;
			$assetsList = $this->get_asset_multiple( $cronNbImages, true );
			if ( count($assetsList) <= 0 ) {
				$ret = array_merge($ret, array(
					'status' 			=> 'done',
					'msg' 				=> 'no assets (to be downloaded) found.',
				));
				return $ret; //false;
			}
			...
			$ret = array_merge($ret, array(
				'status'            => 'done',
				'msg' 				=> sprintf( 'we\ve found %s assets to be downloaded at this step.', count($post_id_list) ),
			));
			return $ret;
		}
		...
		public function restore_assets_bystatus($status='error') {
			global $wpdb;
		...
		public function test_assets() {
			...
			//$this->restore_assets_bystatus('error');
			
			//$this->get_assets_bystatus('inprogress');
			//$this->restore_assets_bystatus('inprogress');
