...
if (class_exists('WooZoneLiteMultipleAmazonKeys') != true) {
	class WooZoneLiteMultipleAmazonKeys
	{
		/*
...
<!-- End Main Wrapper -->
		...
		private function load_available_keys( $keys=false ) {
			...
			foreach ( $keys as $key_idx => $key_info ) {
				...
				if ( isset($key_info['last_request_status']) && ! empty($key_info['last_request_status']) ) {
					...
					$status = sprintf( '<span class="WooZoneLite-message WooZoneLite-%s">%s</span>', $status_css, $status_stat );
				}
				$last_msg = array();
				$last_msg[] = sprintf( __('<u>Status</u> : %s', 'WooZoneLite'), $status );
				...
				if ( isset($key_info['last_request_output']) && ! empty($key_info['last_request_output']) ) {
					...
					if ( ! empty($output) ) {
						$last_msg[] = sprintf( __('<u>Message</u> <br /> %s', 'WooZoneLite'), $output );
					}
				}
				// input
				if ( isset($key_info['last_request_input']) && ! empty($key_info['last_request_input']) ) {
					...
					if ( isset($input['from_file']) ) {
						$last_msg[] = sprintf( __('<u>From File</u> : %s', 'WooZoneLite'), $input['from_file'] );
					}
					if ( isset($input['from_func']) ) {
						$last_msg[] = sprintf( __('<u>From Func</u> : %s', 'WooZoneLite'), $input['from_func'] );
					}
					...
					if ( isset($input['aaAmazonWS'], $input['aaAmazonWS']['requestConfig']) ) {
						...
						$last_msg[] = sprintf( __('<u>Amazon API Request Main Params</u> <br /> %s', 'WooZoneLite'), $reqparams );
					}
				}
				...
				?>
					<tr class="item <?php echo 'Y' == $key_publish ? 'published' : ''; ?>" data-itemid="<?php echo $key_id; ?>">
						<td>
							<ul>
								...
								<li>
									...
									if ( 'N' == $key_publish ) {
									?>
										<input type="button" value="Publish" class="WooZoneLite-button WooZoneLite-form-button-small WooZoneLite-form-button-success WooZoneLite-mk-action-publish">
									<?php
									}
									else {
									?>
										<input type="button" value="Unpublish" class="WooZoneLite-button WooZoneLite-form-button-small WooZoneLite-form-button-warning WooZoneLite-mk-action-publish">
									<?php
									}
									?>
										<input type="button" value="Delete" class="WooZoneLite-form-button-small WooZoneLite-form-button-danger WooZoneLite-mk-action-delete">
								</li>
								<?php
								}
								?>
								<li>
									<input type="button" value="Check Amazon Keys" class="WooZoneLite-form-button-small WooZoneLite-form-button-info WooZoneLite-mk-action-check">
								</li>
							</ul>
						</td>
						<td>
						...
						<td class="WooZoneLite-server-status">
							<ul>
		...
		private function add_key( $pms=array() ) {
			...
			if ( empty($demo_keys) ) {
				return array_replace_recursive($ret, array(
					'msg' 	=> 'Something is wrong!!! There is no aateam demo keys pair!',
				));
			}
			...
			foreach ( $demo_keys as $demokey ) {
				...
				if ( ! in_array( strtoupper(md5(base64_encode($_demokey))), array('1D8CCE0EA0831C8373F042FCC625032B') ) ) {
					return array_replace_recursive($ret, array(
						'msg' 	=> 'Something is wrong!!! You cannot add again an aateam demo keys pair!',
					));
				}
		...
		private function choose_default_keys( $operation='' ) {
			...
			$settings = get_option( $this->the_plugin->alias . '_amazon' ); // 'WooZoneLite_amazon'
			$settings = maybe_unserialize( $settings );
			$settings = !empty($settings) && is_array($settings) ? $settings : array();

			$settings = array_replace_recursive($settings, $found_keys);
			update_option( $this->the_plugin->alias . '_amazon', $settings ); // 'WooZoneLite_amazon'
		}
		...
		public function ajax_request( $retType='die', $pms=array() ) {
			...
			$ret = array(
				'status'        => 'invalid',
				'html'          => '<div class="WooZoneLite-message WooZoneLite-error"><p>' . __('Invalid action!', 'WooZoneLite') . '</p></div>',
			);
