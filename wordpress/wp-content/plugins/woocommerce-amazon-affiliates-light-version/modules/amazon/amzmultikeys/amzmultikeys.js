...
WooZoneLiteAmzMultiKeys = (function($) {
	"use strict";
	...
	(function init() {
		...
		$(document).ready(function() {
			...
			maincontainer = $("#WooZoneLite #WooZoneLite-multikeys");

			// language messages
			lang = maincontainer.find('#WooZoneLite-lang-translation').length
				? maincontainer.find('#WooZoneLite-lang-translation').html()
				: $('#WooZoneLite-wrapper #WooZoneLite-lang-translation').html();
	...
	function load_templates() {
		tpl_main = maincontainer.find('#WooZoneLite-multikeys-tpl');

		// get templates
		var _tpl_delete_confirm		= tpl_main.find('#WooZoneLite-tpl-delete-confirm').html(),
			_tpl_add_confirm		= tpl_main.find('#WooZoneLite-tpl-add-confirm').html();
	...
	function triggers() {
		...
		maincontainer.on('click', '#goto-save-settings', function(e) {
			...
			setTimeout(function() {
				WooZoneLite.scrollToElement( $('.WooZoneLite-saveOptions'), null, { 'verticalOffset' : 0, 'time' : 500 } );
			}, 50);
		});

		//:: reload keys list
		maincontainer.on('click', '.WooZoneLite-mk-action-reload', function(e) {
			e.preventDefault();
		...
		//:: publish key
		maincontainer.on('click', '.WooZoneLite-mk-action-publish', function(e) {
			e.preventDefault();
		...
		//:: delete key
		maincontainer.on('click', '.WooZoneLite-mk-action-delete', function(e) {
			e.preventDefault();
		...
		maincontainer.on('click', '.WooZoneLite-mk-action-delete-no', function(e) {
			e.preventDefault();
		...
		maincontainer.on('click', '.WooZoneLite-mk-action-delete-yes', function(e) {
			e.preventDefault();
		...
		//:: check key
		maincontainer.on('click', '.WooZoneLite-mk-action-check', function(e) {
			e.preventDefault();
		...
		//:: check new key
		maincontainer.on('click', '.WooZoneLite-mk-action-checknew', function(e) {
			e.preventDefault();
		...
		//:: add key
		maincontainer.on('click', '.WooZoneLite-mk-action-addkeys', function(e) {
			e.preventDefault();
		...
		maincontainer.on('click', '.WooZoneLite-mk-action-addkeys-no', function(e) {
			e.preventDefault();
		...
		maincontainer.on('click', '.WooZoneLite-mk-action-addkeys-yes', function(e) {
			e.preventDefault();
	...
	function load_available_keys() {
		...
		data.push({name: 'action', value: 'WooZoneLite_AmzMultiKeysAjax'});
		...
		jQuery.post(ajaxurl, data, function(response) {
			...
			if( response.status == 'valid' ){
				maincontainer.find('.WooZoneLite-multikeys-table > tbody').html( response.html );
			}
	...
	function loading( status, msg ) {
		...
		if ( 'show' == status ) {
			...
			WooZoneLite.to_ajax_loader( msg );
		}
		else {
	...
	function build_check_keys_message( status, html ) {
		...
		*/
		...
		// success
		if ( 'valid' == status ) {
			msg = '<div class="WooZoneLite-message WooZoneLite-success">' + msg + '</div>';
		}
		// error
		else {
			msg = '<div class="WooZoneLite-message WooZoneLite-error">' + msg + '</div>';
		}

		msg = '<div class="WooZoneLite-server-status">' + msg + '</div>';
