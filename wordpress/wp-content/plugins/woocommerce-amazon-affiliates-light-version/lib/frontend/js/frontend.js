...
WooZoneLite = (function ($) {
	...
	function checkout_email() {
		...
		var woozonelite_email_wrapper 		= $('.woozonelite_email_wrapper'),
			  woozonelite_checkout_email 		= $('#woozonelite_checkout_email'),
			  woozonelite_email_mandatory 	= $('#woozonelite_checkout_email_required');

		// checkout email is active
  
		if( woozonelite_checkout_email.length > 0 ) {
			woozonelite_email_wrapper.insertBefore( $('.wc-proceed-to-checkout') );

			var checkout_btn = $('.wc-proceed-to-checkout .checkout-button'),
				  checkout_link = checkout_btn.attr('href');
 
			if( woozonelite_email_mandatory.length > 0 && woozonelite_email_mandatory.val() == '1' ) {
			...
			woozonelite_email_wrapper.on('keyup', woozonelite_checkout_email, function(e) {
				var woozonelite_validate_email = /([A-Z0-9a-z_-][^@])+?@[^$#<>?]+?\.[\w]{2,4}/.test(woozonelite_checkout_email.val());
						 
				 if( woozonelite_validate_email ) {
					...
					if( woozonelite_email_mandatory.length > 0 && woozonelite_email_mandatory.val() == '1' ) {
					 ...
					 woozonelite_checkout_email.css({'border': '1px solid #d1d1d1'});
					...
					woozonelite_checkout_email.css({'border': '1px solid red'});
		...
		$('.wc-proceed-to-checkout').on('click', '.checkout-button', function(e) {
			...
			if ( woozonelite_checkout_email.length > 0 ) {
				...
				if ( woozonelite_checkout_email.val() != '' ) {
					jQuery.post(woozonelite_vars.ajax_url, 
					{
						'action': 'WooZoneLite_before_user_checkout',
						'_nonce': $('#woozonelite_checkout_email_nonce').val(),
						'email': woozonelite_checkout_email.val()
					}, function(data, textStatus) {
						if ( (textStatus === 'success') || (textStatus === 'email_exists') ) {
							//window.location.href = woozonelite_vars.checkout_url;
							$(this).prop('href', woozonelite_vars.checkout_url); // to be sure it does the action!
						} else {
							alert( textStatus );
						}
					});
	...
	var product_country_check = (function() {
		...
		(function init() {
			...
			// load the triggers
			$(document).ready(function(){
				maincontainer = $(".WooZoneLite-country-check:first");
	...
	function syncfront_func() {
		...
		var maincontainer			= null,
			mod_alias 				= 'syncfront/ ',
			sync_wrapper			= '#WooZoneLite-syncfront-wrapper',
			sync_content 			= '#WooZoneLite-syncfront-content',
			sync_debug 				= '#WooZoneLite-syncfront-debug',
		...
		function triggers() {
			...
			maincontainer.on('click', '#WooZoneLite-syncfront-content .WooZoneLite-syncfront-action-refresh-yes', function (e) {
				e.preventDefault();
			...
			maincontainer.on('click', '#WooZoneLite-syncfront-content .WooZoneLite-syncfront-action-refresh-no', function (e) {
				e.preventDefault();
		...
		function get_sync_params() {
			// get params
			var _params = $(sync_wrapper).find('.WooZoneLite-syncfront-params').html();
...
{
	WooZoneLite.async_product_exist( data );
}