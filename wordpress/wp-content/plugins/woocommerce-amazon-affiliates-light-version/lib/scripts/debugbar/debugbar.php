<?php
/*
* Define class WooZoneLiteDebugBar
* Make sure you skip down to the end of this file, as there are a few
* lines of code that are very important.
*/
!defined('ABSPATH') and exit;
	  
if (class_exists('WooZoneLiteDebugBar') != true) { class WooZoneLiteDebugBar {
	 ...
	 * Singleton pattern
	 *
	 * @return WooZoneLiteBitly Singleton instance
	 */
	...
	public function action_admin_bar_menu( $wp_admin_bar ) {
		...
		$title = __( 'WZoneLite Debug Bar', 'woozonelite' );

		$wp_admin_bar->add_menu( array(
			'id'    => 'woozonelite-debugbar',
			'title' => esc_html( $title ),
			'href'  => '#woozonelite-debugbar-dashboard',
		) );

		$wp_admin_bar->add_menu( array(
			'parent' => 'woozonelite-debugbar',
			'id'     => 'woozonelite-debugbar-placeholder',
			'title'  => esc_html( $title ),
			'href'   => '#woozonelite-debugbar-dashboard',
		) );
	}
	...
	public function enqueue_assets() {
		...
		wp_enqueue_style(
			...
			WooZoneLite_asset_path( 'js', $this->the_plugin->cfg['paths']['scripts_dir_url'] . '/debugbar/assets/debugbar.css', true ),
			array( 'dashicons' ),
			WooZoneLite_asset_version( 'css' )
		);

		wp_enqueue_script(
			'woozonelite-debugbar-screenfull',
			WooZoneLite_asset_path( 'js', $this->the_plugin->cfg['paths']['freamwork_dir_url'] . 'js/screenfull/screenfull.min.js', true ),
			array(),
			WooZoneLite_asset_version( 'js' ),
			false
		);
		wp_enqueue_script(
			'woozonelite-debugbar',
			WooZoneLite_asset_path( 'js', $this->the_plugin->cfg['paths']['scripts_dir_url'] . '/debugbar/assets/debugbar.js', true ),
			array( 'jquery-core', 'woozonelite-debugbar-screenfull' ),
			WooZoneLite_asset_version( 'js' ),
			false
		);
	...
	public function show_bar() {
		...
		// dashboard
		$html = array();
		$html[] = '<div id="woozonelite-debugbar-dashboard" class="woozonelite-debugbar-row">';
		$html[] = 	__('This is WZoneLite Debug Bar - Dashboard', 'woozonelite');
		$html[] = '</div>';
	...
	public function can_bar_exist() {
		...
		if ( defined('WOOZONELITE_DEBUGBAR_DISABLED') && WOOZONELITE_DEBUGBAR_DISABLED ) {
			return false;
		}
	...
	// grant the 'view_woozonelite_debug_bar' capability to the user if he can manage options
	...
	public function user_can_view() {
		...
		if ( current_user_can( 'view_woozonelite_debug_bar' ) ) {
			return true;
		}
		return false;
	}
	...
	public function add2bar_generic( $id, $html='', $pms=array() ) {
		...
		switch ( $what ) {
			...
			case 'bar_row':
				...
				$html_ = array();
				$html_[] = '<div id="' . $id . '" class="woozonelite-debugbar-row">';
				$html_[] = $html;
				$html_[] = '</div>';
	...
	public function build_bar_start() {
		...
		$class = array();
		//$class[] = 'woozonelite-debugbar-no-js';

		if ( did_action( 'wp_head' ) ) {
			$class[] = sprintf( 'woozonelite-debugbar-theme-%s', get_template() );
			$class[] = sprintf( 'woozonelite-debugbar-theme-%s', get_stylesheet() );
		}

		if ( ! is_admin_bar_showing() ) {
			$class[] = 'woozonelite-debugbar-force-show';
		}
		//$class[] = 'woozonelite-debugbar-force-show'; //DEBUG
		...
		echo '<div class="woozonelite-debugbar-settings" style="display: none;">' . $bar_settings . '</div>';

		//:: start main box
		echo '<div id="woozonelite-debugbar" class="' . implode( ' ', array_map( 'esc_attr', $class ) ) . '">';

		echo 	'<div id="woozonelite-debugbar-title">';

		echo 		'<img src="' . $this->plugin_icon_url . '" alt="">';
		echo 		'<h1>' . esc_html__( 'WZoneLite Debug Bar', 'woozonelite' ) . '</h1>';

		echo 		'<div class="woozonelite-debugbar-title-heading">';
		echo 			'<select>';

		printf(
							'<option value="%1$s">%2$s</option>',
							'#woozonelite-debugbar-dashboard',
							esc_html__( 'Dashboard', 'woozonelite' )
		);
		...
		echo 		'<div class="woozonelite-debugbar-title-buttons">';
		echo 			'<button class="woozonelite-debugbar-button-fullscreen"><span class="dashicons dashicons-editor-contract" aria-hidden="true" title="' . esc_html__( 'Full Screen', 'woozonelite' ) . '"></span></button>';
		echo 			'<button class="woozonelite-debugbar-button-pin"><span class="dashicons dashicons-paperclip" aria-hidden="true" title="' . esc_html__( 'Pin Box Open', 'woozonelite' ) . '"></span></button>';
		echo 			'<button class="woozonelite-debugbar-button-close"><span class="dashicons dashicons-no" aria-hidden="true" title="' . esc_html__( 'Close Box', 'woozonelite' ) . '"></span></button>';
		echo 		'</div>'; // end title-buttons

		echo 	'</div>'; // end title

		echo 	'<div id="woozonelite-debugbar-wrapper">';
		echo 		'<div id="woozonelite-debugbar-menu">';
		echo 			'<ul>';
		...
		echo 		'<div id="woozonelite-debugbar-rows">';
	}
	...
	public function get_adminbar_menu() {
		...
		$title = esc_html__( 'WZoneLite Debug Bar', 'woozonelite' );
...
// Initialize the WooZoneLiteDebugBar class
//$WooZoneLiteDebugBar = WooZoneLiteDebugBar::getInstance();